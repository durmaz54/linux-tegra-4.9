...
#include "veye_imx335.h"

#define CREATE_TRACE_POINTS

/* veyecam model register address */
#define VEYECAM_MODEL_ID_ADDR		Model_Name
#define VEYECAM_DEVICE_ID 		0x335
...
//32 bit data, 32 bit reg val
static  int veyecam_read_reg(struct i2c_client *client,
				u32 addr, u32 *val)
{
	u32 buf = htonl(addr);
    u32 data;
...
//32 bit data, 32 bit reg val
static int veyecam_write_reg(struct i2c_client *client,
				u32 addr, u32 val)
{
    u8 i2c_w_buf[10];
    int ret = 0;
    debug_printk("write addr %x, val %x",addr,val);
	addr = htonl(addr);
	val = htonl(val);
	memcpy(i2c_w_buf, &addr, 4);
	memcpy(i2c_w_buf + 4, &val, 4);
    
	ret = i2c_master_send(client, i2c_w_buf, 8);
    //should return 8 here
    if(ret >= 0)
        return 0;
    else
        return ret;
...
static int
regmap_util_write_table_32(struct i2c_client *client,
			   ...
			   u32 wait_ms_addr, u32 end_addr)
{
	...
	for (next = table;; next++) {
        ...
        err = veyecam_write_reg(client, next->addr,
                 next->val);
    
        if (err) {
            pr_err("%s:regmap_util_write_table:%d, addr %x val %x",
                   __func__, err,next->addr,
                 next->val);
            return err;
        }
	}
				...
				const veyecam_reg table[])
{
	...
	return regmap_util_write_table_32(i2c_client,
					 table,
...
static int veyecam_set_gain(struct tegracam_device *tc_dev, s64 val)
{
	...
	err = veyecam_write_reg(i2c_client, reg_list[0].addr,
		 reg_list[0].val);
	if (err)
		goto fail;
...
//we will not really power off, to save time!
...
static int veyecam_fuse_id_setup(struct veyecam *priv)
{
	...
	for (i = 0; i < VEYECAM_FUSE_ID_SIZE; i++) {
		err |= veyecam_read_reg(i2c_client,
...
static int veyecam_identify_module(struct veyecam *priv)
{
    ...
    u32 device_id;
    VEYE_TRACE
	ret = veyecam_read_reg(i2c_client, VEYECAM_MODEL_ID_ADDR, &device_id);
	if (ret) {
    ...
    {
        err = 0;
        dev_err(dev, " camera id is VEYE-MIPI-IMX335\n");
    }
    else
    {
        err = -ENODEV;
